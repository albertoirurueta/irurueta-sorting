 ...
 * com.irurueta.sorting.HeapsortSorter
 * 
...
public class HeapsortSorter<T> extends Sorter<T>{
	...
	while(j <= r){
            if(j < r && comparator.compare(ra[j + fromIndex], 
                    ra[j + 1 + fromIndex]) < 0) j++;
            if(comparator.compare(a, ra[j + fromIndex]) >= 0) break;
	...
	double a = ra[l + fromIndex];
        ...
        j = 2 * l + 1;
	while(j <= r){
            if(j < r && ra[j + fromIndex] < ra[j + 1 + fromIndex]) j++;
            if(a >= ra[j + fromIndex]) break;
            ra[jold + fromIndex] = ra[j + fromIndex];
            jold = j;
            j = 2 * j + 1;
	}
